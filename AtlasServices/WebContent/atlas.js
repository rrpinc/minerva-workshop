...
(function(win) {
  ...
  if (win._atlasLoadedJS) return;
  else win._atlasLoadedJS = true;
...
/**
 * atlas CSS/JS form helpers module
 * @module lib/forms.py
 */
...
function jqLiteType(somevar) {
  ...
  if (typeStr.indexOf('[object ') === 0) {
    ...
    throw new Error("atlas: Could not understand type: " + typeStr);
  }    
}
...
function jqLiteOn(element, type, callback, useCapture) {
  ...
  // add to cache
  var cache = element._atlasEventCache = element._atlasEventCache || {};
...
var config = require('../config'),
    ...
    scrollLock = 0,
    scrollLockCls = 'atlas-body--scroll-lock',
    scrollLockPos,
...
function raiseErrorFn(msg) {
  throw new Error("atlas: " + msg);
}


/**
 * Register callbacks on atlasNodeInserted event
...
/**
 * Execute atlasNodeInserted callbacks
 * @param {Event} ev - The DOM event.
 */
function animationHandlerFn(ev) {
  // check animation name
  if (ev.animationName !== 'atlas-node-inserted') return;
...
module.exports = {
  ...
  /** Register atlasNodeInserted handler */
...
var jqLite = require('./lib/jqLite'),
    ...
    attrKey = 'data-atlas-toggle',
    attrSelector = '[data-atlas-toggle="dropdown"]',
    openClass = 'atlas--is-open',
    menuClass = 'atlas-dropdown__menu';
...
function initialize(toggleEl) {
  // check flag
  if (toggleEl._atlasDropdown === true) return;
  else toggleEl._atlasDropdown = true;
...
var jqLite = require('../lib/jqLite'),
    ...
    wrapperClass = 'atlas-select',
    cssSelector = '.atlas-select > select',
    menuClass = 'atlas-select__menu',
    selectedClass = 'atlas--is-selected',
    doc = document,
    win = window;
...
function initialize(selectEl) {
  // check flag
  if (selectEl._atlasSelect === true) return;
  else selectEl._atlasSelect = true;
...
function Menu(wrapperEl, selectEl) {
  ...
  jqLite.scrollTop(this.menuEl, this.menuEl._atlasScrollTop);
...
Menu.prototype._createMenuEl = function(wrapperEl, selectEl) {
  ...
  menuEl._atlasScrollTop = props.scrollTop;
...
/**
 * atlas CSS/JS form-control module
...
var jqLite = require('../lib/jqLite'),
    util = require('../lib/util'),
    cssSelector = '.atlas-textfield > input, .atlas-textfield > textarea',
    emptyClass = 'atlas--is-empty',
    notEmptyClass = 'atlas--is-not-empty',
    dirtyClass = 'atlas--is-dirty',
    floatingLabelClass = 'atlas-textfield--float-label';
...
function initialize(inputEl) {
  // check flag
  if (inputEl._atlasTextfield === true) return;
  else inputEl._atlasTextfield = true;
...
module.exports = {
  ...
  initListeners: function() {
    ...
    setTimeout(function() {
      var css = '.atlas-textfield.atlas-textfield--float-label > label {' + [
        '-webkit-transition',
        '-moz-transition',
        '-o-transition',
        'transition',
...
var util = require('./lib/util'),
    ...
    overlayId = 'atlas-overlay',
    bodyClass = 'atlas--overflow-hidden',
...
function overlayOn(options, childElement) {
  ...
  // attach options
  overlayEl.atlasOptions = options;
...
function overlayOff() {
  ...
  if (overlayEl) {
    ...
    // callback reference
    callbackFn = overlayEl.atlasOptions.onclose;
...
var jqLite = require('./lib/jqLite'),
    util = require('./lib/util'),
    btnClass = 'atlas-btn',
    btnFABClass = 'atlas-btn--fab',
    rippleClass = 'atlas-ripple-effect',
    animationName = 'atlas-btn-inserted';
...
function initialize(buttonEl) {
  // check flag
  if (buttonEl._atlasRipple === true) return;
  else buttonEl._atlasRipple = true;
...
var jqLite = require('./lib/jqLite'),
    ...
    controlsAttrKey = 'data-atlas-controls',
    activeClass = 'atlas--is-active',
    showstartKey = 'atlas.tabs.showstart',
    showendKey = 'atlas.tabs.showend',
    hidestartKey = 'atlas.tabs.hidestart',
    hideendKey = 'atlas.tabs.hideend';
...
function initialize(toggleEl) {
  // check flag
  if (toggleEl._atlasTabs === true) return;
  else toggleEl._atlasTabs = true;
